...
public class ArticleController {
    ...
    @PostMapping("/article")
    public SetArticleDto.Response setArticle(@ModelAttribute SetArticleDto.Request articleRequestDto,
...
//    }
    ...
    @GetMapping("articles")
    public Page<GetArticlesDto.Response> getArticles(@RequestParam("page") int page,
                                                      @RequestParam("size") int size,
                                                      @RequestParam("sortBy") String sortBy,
                                                      @RequestParam("isAsc") boolean isAsc) {
        page = page - 1;
        Page<Article> articles = articleService.getArticles(page , size, sortBy, isAsc);

        Page<GetArticlesDto.Response> response = modelMapper.map(articles, new TypeToken<Page<GetArticlesDto.Response>>() {}.getType());
        return response;
    }
